...
import { IAddingPost } from '../../interfaces/posts';
  ...
  return (
      <Paper style={{ padding: 30 }}>
        <Box className={styles.imageContainer} onClick={() => inputFileRef?.current?.click()}>
          <AddPhotoAlternateIcon color='action' sx={{ fontSize: 80 }} className={clsx(styles.image, { [styles.imageUploaded]: imagePreview })}/>
          {(imagePreview) && (
            <img className={styles.image} src={`http://localhost:4444${imagePreview}`} alt="Uploaded" />
          )}
          <input {...register('imageUrl')} ref={inputFileRef} type="file" onChange={handleChangeFile} hidden  accept="image/png, image/jpeg"/>
        </Box>

        {((isEditing && fullPost.title) || (!isEditing)) && <form onSubmit={handleSubmit(handleSubmitButton)}>
